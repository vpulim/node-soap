...
Client.prototype._isSequenceRequired = function(methodName) {
  ...
  var tnsObject = this.wsdl._splitQName(tns);
  var methodRequestName = _.result(this.wsdl.definitions, 'bindings.'+tnsObject.name+'.methods.'+methodName+'.input.$name');

  // here must be declare method args if declared only params we can't get service methods args and set sequence fields
