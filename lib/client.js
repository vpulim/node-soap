...
class Client extends events_1.EventEmitter {
    ...
    _invoke(method, args, location, callback, options, extraHeaders) {
        ...
        if ((style === 'rpc') && ((input.parts || input.name === 'element') || args === null)) {
            if (!style || style !== 'rpc') {
                throw new Error('AssertionError: invalid message definition for document style binding')
            }
            ...
            if (!style || style !== 'document') {
                throw new Error('AssertionError: invalid message definition for rpc style binding')
            }
        ...
        const eid = options.exchangeId || (isBrowser ? window.crypto.randomUUID() : crypto_1.randomUUID());
