...
Client.prototype.setInterceptor = function(interceptor) {
...
Client.prototype._invoke = function(method, args, location, callback, options, extraHeaders) {
  ...
  function doRequest(xml, callback) {
    var req = self.httpClient.request(location, xml, function(err, response, body) {
      self.lastResponse = body;
      self.lastResponseHeaders = response && response.headers;
  ...
  var interceptor = _.defaults(self.interceptor || {}, {
    processRequest: function(xml, cb) {
      cb(null, xml);
    },
    processResponse: function(xml, response, cb) {
      cb(null, xml, response);
    }
  });

  async.waterfall([
    function(cb) {
      interceptor.processRequest(xml, cb);
    },
    doRequest,
    interceptor.processResponse,
    parseSync
  ], function (err, result) {
    callback(err, result);
  });
