...
Client.prototype._invoke = function(method, args, location, callback, options, extraHeaders) {
  ...
  const headerAttributes = Object.keys(this.wsdl.options.headerAttributes).map(attribute => `${attribute}="${this.wsdl.options.headerAttributes[attribute]}"`).join(' ');
  xml = "<?xml version=\"1.0\" encoding=\"utf-8\"?>" +
    ...
    ((self.soapHeaders || self.security) ?
      (
        "<" + envelopeKey + ":Header " + headerAttributes + ">" +
