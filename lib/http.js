...
const axios_ntlm_1 = require("noop");
...
class HttpClient {
    ...
    buildRequest(rurl, data, exheaders, exoptions = {}) {
        ...
        if (exoptions.forceMTOM || attachments.length > 0) {
            const start = isBrowser ? window.crypto.randomUUID() : crypto_1.randomUUID();
            let action = null;
            ...
            const boundary = isBrowser ? window.crypto.randomUUID() : crypto_1.randomUUID();
