...
Server.prototype._process = function(input,URL) {
    ...
    var self = this,
		...
		if(Object.keys(bindings).length===1) {
		    binding = bindings[Object.keys(bindings)[0]];
		} else {
		...
		if(binding.style === 'rpc') {
			...
			return self._executeMethod(methodName, methodName+'Response', obj[methodName], 'rpc');
		} else {
			var messageName = Object.keys(obj)[0];
	    for (methodName in methods) {
	        method = methods[methodName];
	        if (method.input.$name === messageName) {
	            return self._executeMethod(methodName, method.output.$name, obj[messageName], 'document');
	        }
			}
		}
...
Server.prototype._executeMethod = function(methodName, outputName, args, style) {
    var services = this.services, body;
    for (var serviceName in services) {
        var service = services[serviceName];
