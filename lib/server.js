...
function getPathsFromWSDL(wsdl, caseInsensitivePaths, versionAsSuffix) {
  var paths = [];
  var services = wsdl.definitions.services;
  var name;
  for (name in services) {
    ...
    for (name in ports) {
      var port = ports[name];
      var location = url.parse(port.location).pathname.replace(/\/$/, '');

      if (caseInsensitivePaths)
        location = location.toLowerCase()
      if (versionAsSuffix)
        location = pathParser.resolve(location, '..')
...
var Server = function(server, path, services, wsdl, options) {
  ...
  var caseInsensitivePaths = options.caseInsensitivePaths || false;
  var versionAsSuffix = options.versionAsSuffix || false;

  wsdl.onReady(function(err) {
    var listeners = server.listeners('request').slice();

    // get path(s) from WSDL
    var paths = []
    if (path == null) {
      paths = getPathsFromWSDL(wsdl, caseInsensitivePaths, versionAsSuffix)
      //console.log('Getting paths from WSDL: '+paths)
    } else {
    ...
    server.addListener('request', function(req, res) {
      ...
      if (versionAsSuffix)
        reqPath = pathParser.resolve(reqPath, '..')
