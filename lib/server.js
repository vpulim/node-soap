...
Server.prototype._process = function(input, URL, callback) {
  ...
  binding = (function(self) {
    ...
    for (_serviceName in services) {
      serviceName = _serviceName;
      service = services[_serviceName];
      ports = service.ports;
      for (_portName in ports) {
        portName = _portName;
        port = ports[_portName];
        portPathname = url.parse(port.location).pathname.replace(/\/$/, '');

        if (typeof self.log === 'function') {
          self.log("info", "Trying " + _portName + " from path " + portPathname);
        }
    ...
    serviceName = _serviceName;
    portName = _portName;
    return !firstPort ? undefined : firstPort.binding;
  })(this);
