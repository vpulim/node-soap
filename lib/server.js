...
Server.prototype._executeMethod = function(options, callback, includeTimestamp) {
  ...
  function handleResult(result, fault, statusCode) {
    if (handled)
      return;
    handled = true;

    if (fault && fault.Fault !== undefined) {
      // 3rd param is the NS prepended to all elements
      // It must match the NS defined in the Envelope (set by the _envelope method)
      body = self.wsdl.objectToDocumentXML("Fault", fault.Fault, "soap");
    } else if (style === 'rpc') {
    ...
    callback(self._envelope(body, includeTimestamp), statusCode);
  }
