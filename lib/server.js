    ...
    if (req.method === 'GET') {
        ...
        req.on('end', function() {
            ...
            try {
                ...
                var report = err.message;
                var body = "<soap:Fault>" +
                    "<faultcode type=\"string\">soap:Client</faultcode>" +
                    "<faultstring type=\"string\">" + report + "</faultstring>" +
                    "</soap:Fault>";
                var output = self._envelope(body);
                res.write(output);
                res.end();
                if (typeof self.log === 'function') {
                  self.log("error", err);
...
Server.prototype._executeMethod = function(options, callback) {
    ...
    function handleResult(err, result) {
        ...
        if (err) {
            ...
            var errMsg = "<soap:Fault>" +
                "<faultcode type=\"string\">soap:Server</faultcode>" +
                "<faultstring type=\"string\">" + report + "</faultstring>" +
                "</soap:Fault>";
            return callback(self._envelope(errMsg));
        }
