...
Server.prototype._executeMethod = function(options, req, callback, includeTimestamp) {
  ...
  var self = this,
    ...
    args = options.args,
    style = options.style,
    isSoap12 = options.isSoap12,
    handled = false;

  try {
    method = this.services[serviceName][portName][methodName];
  } catch (error) {
    return callback(this._envelope('', includeTimestamp, isSoap12));
  }
  ...
  function handleResult(error, result) {
    ...
    callback(self._envelope(body, includeTimestamp, isSoap12));
  }
...
Server.prototype._envelope = function(body, includeTimestamp, isSoap12) {
