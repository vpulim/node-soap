  ...
  binding = (function(self) {
    ...
    for (name in services) {
      ...
      for (name in ports) {
        ...
        var parsedPortUrl = url.parse(port.location, true);
        var portPathname = parsedPortUrl.pathname;
        if (typeof self.log === 'function') {
          self.log("info", "Trying " + portName + " from path " + portPathname);
        }

        if (portPathname === pathname
          && _.isEqual(_.pick(parsedUrl.query, _.keys(parsedPortUrl.query)), parsedPortUrl.query))
          return port.binding;
