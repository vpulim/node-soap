...
for (var n in ElementTypeMap) {
  if (ElementTypeMap.hasOwnProperty(n)) {
    ...
    if (this.element.$type) {
      ...
      if (typeNs) {
        if (type.name in Primitives) {
          if (Primitives.hasOwnProperty(type.name)) {
...
ExtensionElement.prototype.description = function(definitions, xmlns) {
  ...
  if (this.$base) {
    ...
    if (typeName in Primitives) {
      if (Primitives.hasOwnProperty(typeName)) {
        return this.$base;
      }
    }
    else {
    ...
    self.processIncludes(function(err) {
      ...
      if (complexTypes) {
        for (name in complexTypes) {
          if (complexTypes.hasOwnProperty(name)) {
...
WSDL.prototype.xmlToObject = function(xml, callback) {
  ...
  p.onopentag = function(node) {
    ...
    for(attributeName in elementAttributes){
      if (elementAttributes.hasOwnProperty(attributeName)) {
...
WSDL.prototype.processAttributes = function(child, nsContext) {
  ...
  if (attrObj) {
    for (var attrKey in attrObj) {
      if (!attrObj.hasOwnProperty(attrKey)) continue;
...
WSDL.prototype._xmlnsMap = function() {
  ...
  for (var alias in xmlns) {
    if (!xmlns.hasOwnProperty(alias)) continue;
