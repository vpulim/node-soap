...
Element.prototype.deleteFixedAttrs = function() {
  ...
  if (this.xmlns && !Object.keys(this.xmlns).length)
    delete this.xmlns;
  delete this.nsName;
...
MessageElement.prototype.postProcess = function(definitions) {
  ...
  if (part.$element) {
    ...
    ns = nsName.namespace;
    this.element = definitions.schemas[
    ...
    this.parts = {};
    delete this.element;
    for (i = 0, part; part = this.children[i]; i++) {
      assert(part.name === 'part', 'Expected part element');
      nsName = splitNSName(part.$type);
      ns = definitions.xmlns[nsName.namespace];
      type = nsName.name;
      schemaDefinition = definitions.schemas[ns];
...
var WSDL = function(definition, uri, options) {
  ...
  process.nextTick(function() {
    ...
    self.processIncludes(function(err) {
      ...
      services = self.services = self.definitions.services;
      if (services) {
        for (name in services) {
          services[name].postProcess(self.definitions);
        }
      }
      complexTypes = self.definitions.complexTypes;
      if (complexTypes) {
      ...
      bindings = self.definitions.bindings;
...
WSDL.prototype.objectToXML = function(obj, name, namespace, xmlns, first) {
  ...
  if (Array.isArray(obj)) {
    for (i = 0; item = obj[i]; i++) {
      if (i > 0) {
        parts.push(['</', ns, name, '>'].join(''));
        parts.push(['<', ns, name, xmlnsAttrib, '>'].join(''));
      }
      parts.push(this.objectToXML(item, name, namespace, xmlns));
    }
  }
    ...
    for (name in obj) {
      child = obj[name];
      parts.push(['<', ns, name, xmlnsAttrib, '>'].join(''));
      parts.push(this.objectToXML(child, name, namespace, xmlns));
      parts.push(['</', ns, name, '>'].join(''));
    }
