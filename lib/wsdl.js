...
// check if schema namespaces need to be ignored
WSDL.prototype.ignoreNamespace = function(schema) {
...
WSDL.prototype.objectToXML = function(obj, name, namespace, xmlns, first, xmlnsAttr, parameterTypeObject, ancestorXmlns) {
  ...
  var ignoreNamespace = this.ignoreNamespace(schema);

  var xmlnsAttrib = '';
  if (xmlns && first) {

    if (prefixNamespace && (!ignoreNamespace || this.ignoredNamespaces.indexOf(namespace) === -1)) {
  ...
  var ns = '';
  if (prefixNamespace && ((qualified || first) || soapHeader) && (!ignoreNamespace || this.ignoredNamespaces.indexOf(namespace) === -1)) {
    // prefix element
