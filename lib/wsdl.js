...
WSDL.prototype.xmlToObject = function(xml) {
  ...
  if (body.Fault) {
    var msg, detail;
    if (body.Fault.faultcode) {
      // Soap 1.1
      msg = body.Fault.faultcode + ': ' + body.Fault.faultstring;
      detail = body.Fault.detail;
    } else {
      // Soap 1.2
      msg = body.Fault.Code.Value + ': ' + body.Fault.Reason.Text.$value;
      detail = body.Fault.Detail;
    }
    if (detail) {
      msg = msg + ': ' + inspect(detail);
    }
