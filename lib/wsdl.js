...
WSDL.prototype.objectToDocumentXML = function(name, params, ns, xmlns) {
    ...
    args[name] = params;
    /*
     * NOTE:  Remove the final true parameter to the objectToXML call
     * here because namespace definitions should not appear in the
     * body, and this method is only called when generating SOAP
     * bodies (client.js and server.js)
     */
...
WSDL.prototype.objectToXML = function(obj, name, namespace, xmlns, first) {
    ...
    if (Array.isArray(obj)) {
        /*
         * Don't include namespace prefix if schema attribute elementFormDefault is
         * set to 'unqualified'
         */
        var prefix = ns;
        if( thisSchema.$elementFormDefault == 'unqualified' ) {
           prefix = '';
        }
        ...
        for (var name in obj) {
            ...
            var child = obj[name];
            /*
             * Don't include namespace prefix if the schema attribute
             * elementFormDefault is set to 'unqualified' AND this is
             * a lead-node child
             */
            if( thisSchema.$elementFormDefault == 'unqualified'
               && typeof child != 'object'
