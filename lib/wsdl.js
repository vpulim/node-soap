...
WSDL.prototype.objectToDocumentXML = function(name, params, ns, outStream, callback) {
    var args = {};
    args[name] = params;
    this.objectToXML(args, null, ns, outStream, callback);
}

WSDL.prototype.objectToRpcXML = function(name, params, namespace, xmlns, outStream, callback) {
    var self = this
    ...
    outStream.write(['<',namespace,':',name,'>'].join(''));
    ...
    self.objectChildToRpcXML(children, name, params, namespace, xmlns, outStream, function () {
      outStream.write(['</',namespace,':',name,'>'].join(''));
      callback();
    });
}

WSDL.prototype.objectChildToRpcXML = function(children, name, params, namespace, xmlns, outStream, callback) {
  var self = this
      , defs = this.definitions
      , nsAttrName = '_xmlns'
      , key = children.shift()
      , newCallback = function () {
          outStream.write(['</',key,'>'].join(''));
          callback();
        };
  ...
  self.objectToXML(params[key], '', '', outStream, newCallback);
}

WSDL.prototype.objectToXML = function(obj, name, namespace, outStream, callback) {
    var self = this,
        ns = namespace ? namespace + ':' : '';

    if (Array.isArray(obj)) {
      self.ArrayToXML(obj, 0, name, namespace, outStream, callback);
    } else if (typeof obj === 'object') {
      if(obj instanceof stream.Stream) {
        return self._streamToStream(obj, outStream, callback);
      } else {
        return self.RealObjectToXML(obj, name, namespace, outStream, callback);
      }
    } else if (typeof(obj) === 'string') {
      outStream.write(xmlEscape(obj));
      return callback();
    } else if (obj) {
      // Streams only take strings and buffers
      outStream.write(String(obj));
    }
    return callback();
}

WSDL.prototype.RealObjectToXML = function(obj, name, namespace, outStream, callback) {
    var self = this
      , children = [];
    ...
    self.RealObjectChildToXML(children, name, namespace, outStream, callback);
}

WSDL.prototype.RealObjectChildToXML = function(children, name, namespace, outStream, callback) {
    var self = this
      , ns = namespace ? namespace + ':' : ''
    ...
    outStream.write(['<',ns,child.name,'>'].join(''));
    self.objectToXML(child.child, child.name, namespace, outStream, function () {
      outStream.write(['</',ns,child.name,'>'].join(''));
      self.RealObjectChildToXML(children, name, namespace, outStream, callback);
    });
}

WSDL.prototype.ArrayToXML = function(obj, i, name, namespace, outStream, callback) {
    var self = this
      , ns = namespace ? namespace + ':' : ''
      , item=obj[i];
    ...
    if (start > 0) {
        outStream.write(['</',ns,name,'>'].join(''));
        outStream.write(['<',ns,name,'>'].join(''));
    }

    i++;
    self.objectToXML(item, name, namespace, outStream, function () {
      self.ArrayToXML(obj, i, name, namespace, outStream, callback);
    });
}

WSDL.prototype._streamToStream = function(inStream, outStream, callback ) {
  inStream.on('data', function(data) {
    outStream.write(data);
  });

  inStream.on('error', function() {
    outStream.emit('error', new Error('Unexpected error when streaming from a stream in arguments'));
  });

  // When streaming is done, proceed
