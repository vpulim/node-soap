...
function overrideSchemaNamespacesWithDefinitions(schemaXmlns, definitionsXmlns, xmlns) {
  var invXmlns = _.invert(xmlns || {});
  var invSchema = _.invert(schemaXmlns);
  var invDefinitions = _.invert(definitionsXmlns);
  var merged = Object.assign({}, invSchema, invDefinitions, xmlns);
  return _.invert(merged);
}
...
ElementElement.prototype.description = function(definitions, xmlns) {
  ...
  if (type) {
    ...
    ns = xmlns && xmlns[type.prefix] || /* this.schemaXmlns[type.prefix] || */ definitions.xmlns[type.prefix],
    schema = definitions.schemas[ns],
...
WSDL.prototype._parse = function(xml) {
  ...
  p.onopentag = function(node) {
    ...
    var top = stack[stack.length - 1];
    var name;
    if (splitQName(nsName).name === 'schema') {
      schemaAttrs = attrs;
    }
    if (top) {
      try {
