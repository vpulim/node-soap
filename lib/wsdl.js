...
var WSDL = function(definition, uri, options) {
  ...
  if (this.options.attributesNamespace === undefined) {
    this.options.attributesNamespace = '';
  }
...
WSDL.prototype.xmlToObject = function(xml) {
  ...
  p.onopentag = function(node) {
    ...
    if(hasNonXmlnsAttribute)obj[self.getAttributesKey()] = elementAttributes;
...
WSDL.prototype.objectToXML = function(obj, name, namespace, xmlns, first, xmlnsAttr, parameterTypeObject, ancestorXmlns) {
  ...
  if (Array.isArray(obj)) {
    ...
    for (name in obj) {
      ...
      if (name === self.getAttributesKey()) {
        continue;
      }

      var child = obj[name];
      if (child[self.getAttributesKey()] && child[self.getAttributesKey()].xsi_type) {
        var xsiType = child[self.getAttributesKey()].xsi_type;
      ...
      if (first) {
        ...
        if (self.definitions.schemas) {
          ...
          if (schema) {
            ...
            } else if (obj[self.getAttributesKey()] && obj[self.getAttributesKey()].xsi_type) { //if parent object has complex type defined and child not found in parent
              var completeChildParamTypeObject = self.findChildParameterObjectFromSchema(obj[self.getAttributesKey()].xsi_type.type, obj[self.getAttributesKey()].xsi_type.xmlns);

              ns = obj[self.getAttributesKey()].xsi_type.namespace + ':';
              ancXmlns.push(obj[self.getAttributesKey()].xsi_type.xmlns);
              value = self.objectToXML(child, name, obj[self.getAttributesKey()].xsi_type.namespace, obj[self.getAttributesKey()].xsi_type.xmlns, false, null, null, ancXmlns);
            } else {
...
WSDL.prototype.processAttributes = function(child) {
  var attr = '';
  if (child[this.getAttributesKey()]) {
    for (var attrKey in child[this.getAttributesKey()]) {
      //handle complex extension separately
      if (attrKey === 'xsi_type') {
        var attrValue = child[this.getAttributesKey()][attrKey];
        attr += ' xsi:type="' + attrValue.namespace + ':' + attrValue.type + '"';
        attr += ' xmlns:' + attrValue.namespace + '="' + attrValue.xmlns + '"';

        continue;
      } else {
        attr += ' ' + attrKey + '="' + xmlEscape(child[this.getAttributesKey()][attrKey]) + '"';
      }
    }
  }
...
WSDL.prototype.getAttributesKey = function () {
  return this.options.attributesNamespace + 'attributes';
};
