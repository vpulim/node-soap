...
WSDL.prototype.objectToXML = function(obj, name, namespace, xmlns, first, xmlnsAttr, parameterTypeObject, ancestorXmlns) {
  var self = this;
  var schema = this.definitions.schemas[xmlns];
  var unqualified = schema && schema.$elementFormDefault === 'unqualified';
  var parts = [];

  var xmlnsAttrib = (first && xmlns)
    ? ((namespace && namespace !== 'xmlns' ? ' xmlns:' + namespace + '="' + xmlns + '"' : '') + (!unqualified ? ' xmlns="' + xmlns + '"' : ''))
    : '';
  ...
  var ns = '';
  if (namespace && namespace !== 'xmlns' && !unqualified) {
