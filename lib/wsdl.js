...
WSDL.prototype.objectToRpcXML = function(name, params, namespace, xmlns) {
  ...
  var elementFormDefaultQualified = self.isElementFormQualified(xmlns);
  namespace = namespace || findKey(defs.xmlns, xmlns);
  xmlns = xmlns || defs.xmlns[namespace];
  
  var emptyNameSpace = self.options.forceEmptyNamespace ? ' xmlns=""' : '';
  namespace = namespace === 'xmlns' ? emptyNameSpace : (namespace + ':');

  parts.push(['<', elementFormDefaultQualified ? namespace: '', name, '>'].join(''));
...
WSDL.prototype.objectToXML = function(obj, name, namespace, xmlns, first, xmlnsAttr, parameterTypeObject, ancestorXmlns) {
  ...
  var parts = [];
  var elementFormDefaultQualified = self.isElementFormQualified(xmlns);
  var emptyNameSpace = !elementFormDefaultQualified && self.options.forceEmptyNamespace ? ' xmlns=""' : '';
  ...
  var ns = namespace && namespace !== 'xmlns' ? (namespace.indexOf(":") === -1 ? namespace + ':' : namespace) : emptyNameSpace;
  if (Array.isArray(obj)) {
    

    for (var i = 0, item; item = obj[i]; i++) {
      var arrayAttr = self.processAttributes(item);
      parts.push(['<', elementFormDefaultQualified ? ns : '', name, arrayAttr, xmlnsAttrib, '>'].join(''));
      parts.push(self.objectToXML(item, name, namespace, xmlns, false, null, null, ancXmlns));
      parts.push(['</', elementFormDefaultQualified ? ns : '', name, '>'].join(''));
    }
    ...
    for (name in obj) {
      ...
      if (!Array.isArray(child)) {
        ...
        parts.push(['<', elementFormDefaultQualified ? ns : '', name, attr, xmlnsAttrib, '>'].join(''));
      }
...
WSDL.prototype.isElementFormQualified = function(xmlns){
  var self = this;
  var thisSchema = self.definitions.schemas[xmlns];
  if(thisSchema && thisSchema.$elementFormDefault === 'qualified') {
    return true;
  }
  return false;
};
