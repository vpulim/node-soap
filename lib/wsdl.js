...
function splitNSName(nsName, name) {
    if (name && !(nsName && (nsName instanceof String))) {
        // Evidently we sometimes do not get a namespace prefixed name at all
...
WSDL.prototype.xmlToObject = function(xml) {
    ...
    p.on('text', function(text) {
        ...
        /*
          If a nested element lacks namespace prefix, top.schema ends up being an empty object.
          Send the along the name as default.
         */
        var name = splitNSName(top.schema, top.name).name,
            value;
        if (name === 'int') {
