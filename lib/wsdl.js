  ...
  p.onopentag = function(node) {
    ...
    if (!objectName && top.name === 'Body' && name !== 'Fault') {
      ...
      if (!message) {
        ...
        var isInput = false;
        var isOutput = false;
        if ((/(?:Response|ResponseMsg)$/).test(name)) {
          isOutput = true;
          name = name.replace(/(?:Response|ResponseMsg)$/, '');
        } else if ((/(?:Request|RequestMsg|Solicit)$/).test(name)) {
          isInput = true;
          name = name.replace(/(?:Request|RequestMsg|Solicit)$/, '');
        }
