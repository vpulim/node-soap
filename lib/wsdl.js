...
MessageElement.prototype.postProcess = function(definitions) {
  ...
  if (part.$element) {
    ...
    if (this.element.$type) {
        var type = splitNSName(this.element.$type); 
    ...
    for (i = 0; part = this.children[i]; i++) {
      ...
      if (typeof schemaDefinition !== 'undefined') {
        this.parts[part.$name] = definitions.schemas[ns].types[nsName.name] || 
          definitions.schemas[ns].complexTypes[nsName.name];
      } else {
...
WSDL.prototype.xmlToObject = function(xml) {
  ...
  p.onopentag = function(node) {
    ...
    var elemSchema = topSchema && topSchema[name];

    // format attributes according to their types
    if(hasNonXmlnsAttribute)obj.attributes = elementAttributes;

    stack.push({name: originalName, object: obj, schema: elemSchema, id: attrs.id});
  };
