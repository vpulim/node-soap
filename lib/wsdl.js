...
var ElementTypeMap = {
    ...
    complexType: [ComplexTypeElement,  'annotation sequence all complexContent'],
    complexContent: [ComplexContentElement, 'extension'],
    extension: [ExtensionElement, 'annotation sequence all'],
...
ComplexContentElement.prototype.description = function(definitions) {
    var children = this.children;
    for (var i=0, child; child=children[i]; i++) {
...
ExtensionElement.prototype.description = function(definitions) {
    var elements = {},
        baseName = splitNSName(this.$base).name,
        ns = definitions.xmlns[splitNSName(this.$base).namespace],
        schema = definitions.schemas[ns],
        typeElement = schema && ( schema.complexTypes[baseName] || schema.types[baseName] );
    if (typeElement && !(baseName in Primitives)) {
        elements = typeElement.description(definitions);
    }
    ...
    for (var i=0, child; child=children[i]; i++) {
        if (child instanceof SequenceElement ||
            child instanceof AllElement) {
            elements = extend(child.description(definitions), elements);
            break;
        }
    }
...
WSDL.prototype.xmlToObject = function(xml) {
    ...
    p.on('text', function(text) {
        ...
        if (top && top.schema) {
            name = splitNSName(top.schema.type).name;
        }

        text = trim(text);
        if (!text.length) {
            if (!top.schema.nillable) {
                throw new Error("Required parameter " + top.name + " is not specified!");
            }
            return;
        }
        ...
        if (name === 'int' || name === 'integer') {
            ...
            var isTrue = text.toLowerCase() === 'true' || text === '1';
            var isFalse = text.toLowerCase() === 'false' || text === '0';
            if (isTrue) {
                value = true;
            } else if (isFalse) {
                value = false;
        ...
        if (testNaN && isNaN(value) && !top.schema.nillable) {
            throw new Error("Required parameter " + top.name + " (" + name + ") has the wrong value: " + text);
        }
