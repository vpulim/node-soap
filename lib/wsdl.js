...
ComplexTypeElement.prototype.description = function(definitions, cache, key) {
    var children = this.children;
    for (var i=0, child; child=children[i]; i++) {
        if (child instanceof SequenceElement ||
            child instanceof AllElement) {
            var desc, replaceRecursionPlaceholder;

            // stores marker for detected recursions
            cache = cache || {};

            if (cache.hasOwnProperty(key)) {
                // mark this key as recursive
                cache[key] = true;
                return {recursionPlaceholder: key};
            }

            // recursion sentinel
            cache[key] = false;

            desc = child.description(definitions, cache, key);

            // has there been a recursion
            if (cache[key] === true) {
                // traverse description and replace each recursionPlaceholder for this key
                // with this description
                (replaceRecursionPlaceholder = function(obj) {
                    var prop, value;
                    for (prop in obj) {
                        value = obj[prop];
                        if(value != null && Object.prototype.toString.call(value) === '[object Object]') {
                            // either another complex type or the recursion placeholder
                            if (value.recursionPlaceholder === key) {
                                obj[prop] = desc;
                            } else {
                                // dive into that
                                replaceRecursionPlaceholder(value);
                            }
                        }
                    }
                })(desc);
...
ElementElement.prototype.description = function(definitions, cache) {
    var element = {},
        name = this.$name,
        // key is used to detect recursion in complex types
        key = this.$type + '$' + name,
        schema;
    ...
    if (this.$type) {
        ...
        if (typeElement && !(typeName in Primitives)) {
            element[name] = typeElement.description(definitions, cache, key);
        }
        else
        ...
        for (var i=0, child; child=children[i]; i++) {
            if (child instanceof ComplexTypeElement)
                element[name] = child.description(definitions, cache, key);
        }
    }
    return element;
}
AllElement.prototype.description =
SequenceElement.prototype.description = function(definitions, cache, key) {
    var children = this.children;
    var sequence = {};
    for (var i=0, child; child=children[i]; i++) {
        var description = child.description(definitions, cache, key);
...
WSDL.prototype.xmlToObject = function(xml) {
    ...
    p.on('endElement', function(nsName) {
        var cur = stack.pop(),
						obj = cur.object,
            top = stack[stack.length-1],
            topObject = top.object || (top.object = {}),
            topSchema = top.schema,
