...
function listen(server, pathOrOptions, services, xml, callback) {
    ...
    var xmlOrUrl = xml || services;

    if (xmlOrUrl.charAt(0) === '<') {
        var wsdl = new WSDL(xml || services, null, options);
        return new Server(server, path, services, wsdl);
    }

    _requestWSDL(xmlOrUrl, options, function(err, wsdl) {
        callback(err, wsdl && new Server(server, path, services, wsdl, true));
    });
}
