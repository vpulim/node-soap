...
function listen(server, pathOrOptions, callback, services, xml) {
    var options = {},
        path = pathOrOptions,
        uri;

    // new optional callback
    if(typeof callback !== 'function'){xml=services;services=callback;callback=void 0}
    ...
    if(uri){
        ...
        _requestWSDL(uri,options,function(err,wsdl_requested){
            if(err) return cb(err);
            callback(null,new Server(server, path, services, wsdl_requested));
            /* wdsl should have been an event emitter, this is a work around to add the server listener after wsdl is finally loaded */
            process.nextTick(function() {
                wsdl_requested.callback(null,wsdl_requested);
            });
        });
    } else {
        var wsdl = new WSDL(xml || services, null, options);
        var server = new Server(server, path, services, wsdl);
