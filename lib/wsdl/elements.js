    ...
    postProcess(definitions) {
        ...
        if (part.$element) {
            ...
            for (let i = 0; part = this.children[i]; i++) {
                ...
                if (part.name !== 'part') {
                    throw new Error('AssertionError: Expected part element');
                }
...
class SchemaElement extends Element {
    ...
    merge(source) {
        if (!(source instanceof SchemaElement)) {
            throw new Error('AssertionError: Expected source to be a SchemaElement');
        }
...
class TypesElement extends Element {
    ...
    addChild(child) {
        if (!(child instanceof SchemaElement)) {
            throw new Error('AssertionError: Expected child to be a SchemaElement');
        }
