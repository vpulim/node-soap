...
describe('WSDL Parser (strict)', () => {
  ...
  it('should catch parse error', (done) => {
    soap.createClient(__dirname + '/wsdl/bad.txt', { strict: true }, function (
      err
    ) {
  ...
  it('should describe extended elements in correct order', (done) => {
    ...
    soap.createClient(__dirname + '/wsdl/extended_element.wsdl', function (
      err,
      client
    ) {
      assert.ifError(err);
  ...
  it('should handle element ref', (done) => {
    ...
    soap.createClient(
      ...
      function (err, client) {
        assert.ifError(err);
        client.fooOp(
          { paymentRq: { bankSvcRq: { requestUID: '001' } } },
          function (err, result) {
  ...
  it('should parse POJO into xml without making unnecessary recursion', (done) => {
    ...
    soap.createClient(
      __dirname + '/wsdl/perf/order.wsdl',
      { strict: true },
      function (err, client) {
        var i, spyCall;
  ...
  it('should merge schema with attributes', (done) => {
    ...
    soap.createClient(
      __dirname + '/wsdl/mergeWithAttributes/main.wsdl',
      {},
      function (err, client) {
        assert.ok(!err);
...
describe('WSDL Parser (non-strict)', () => {
  ...
  it('should all attributes to root elements', (done) => {
    ...
    soap.createClient(__dirname + '/wsdl/elementref/foo.wsdl', {}, function (
      err,
      client
    ) {
      assert.ok(!err);
      client.fooOp(
        {
          paymentRq: {
            attributes: { 'bar1:test': 'attr' },
            bankSvcRq: { ':requestUID': '001' },
          },
        },
        function (err, result) {
          assert.equal(client.lastMessage, expectedMsg);
  ...
  it('should describe recursive wsdl with extended elements', (done) => {
    soap.createClient(__dirname + '/wsdl/extended_recursive.wsdl', function (
      err,
      client
    ) {
      assert.ifError(err);
      ...
      var personDescription =
        desc.Service1.BasicHttpBinding_IService1.GetPerson.output
          .GetPersonResult;
      assert.equal(
        personDescription,
        personDescription.Department.HeadOfDepartment
      );
      done();
    });
  });
  ...
  it('should describe referenced elements with type of the same name', (done) => {
    soap.createClient(
      __dirname + '/wsdl/ref_element_same_as_type.wsdl',
      function (err, client) {
        assert.ifError(err);
        var desc = client.describe();
        assert.equal(
          desc.MyService.MyPort.MyOperation.input.ExampleContent.MyID,
          'xsd:string'
        );
        done();
      }
  ...
  it('should describe port type', (done) => {
    soap.createClient(
      ...
      function (err, client) {
        assert.ifError(err);
        var desc = client.wsdl.definitions.portTypes.MyPortType.description(
          client.wsdl.definitions
        );
